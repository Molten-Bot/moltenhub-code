...
func TestRunPRCreateAuthRequiredAfterPushCompletesWithBranch(t *testing.T) {
	t.Parallel()
	...
	fake := &fakeRunner{t: t, allowUnorderedClones: true, exps: []expectedRun{
		...
		{cmd: commitCommand(repoDir, cfg.CommitMessage)},
		{cmd: pushCommand(repoDir, branch)},
		{cmd: prCreateCommand(repoDir, cfg, branch), res: execx.Result{Stderr: prCreateStderr}, err: errors.New("exit status 1")},
	}}
	...
	if !strings.Contains(strings.Join(logs, "\n"), "action=manual_create_required reason=create_pull_request_auth_required") {
		t.Fatalf("logs missing auth-required manual PR warning:\n%s", strings.Join(logs, "\n"))
	}
	if len(fake.exps) != 0 {
		t.Fatalf("unconsumed expectations: %d", len(fake.exps))
	}
