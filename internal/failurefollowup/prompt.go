...
const ValidationToolingInstruction = "If local test or validation tooling is unavailable in this runtime (for example `command not found` or missing `node_modules`), do not fail solely for that and do not use `Failure:` solely for that validation gap. If dependency installation cannot reach a package registry and requested repository work is otherwise complete, treat resulting unavailable test, lint, typecheck, and build commands as a validation gap, not a task failure. This exception does not apply when installing or updating dependency or package files is requested work. Continue with repository changes, run any alternative checks you can, and clearly report the validation gap."
