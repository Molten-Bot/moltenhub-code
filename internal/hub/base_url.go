...
// ValidatePersistedHubBaseURL validates hub base URLs that may be written to
// runtime config. Persisted config must stay on HTTPS Molten Hub domains even
// when local non-Molten endpoints are allowed for in-process tests.
func ValidatePersistedHubBaseURL(baseURL string) error {
	...
	if !hasSuffix || strings.Contains(region, ".") || strings.TrimSpace(region) == "" {
		return fmt.Errorf("base_url host must be a Molten Hub domain")
	}
