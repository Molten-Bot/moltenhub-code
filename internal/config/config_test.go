...
func TestApplyDefaultsAppendsMoltenBotHubFooterToDefaultPRBody(t *testing.T) {
	...
	if !strings.Contains(cfg.PRBody, prBodyGeneratedByLine) {
		t.Fatalf("PRBody = %q, want generated-by line to use product name Molten Hub Code", cfg.PRBody)
	}
	if strings.Contains(cfg.PRBody, "Automated change generated by MoltenHub Code.") {
		t.Fatalf("PRBody = %q, want generated-by line to avoid unspaced product name", cfg.PRBody)
	}
	if !strings.Contains(cfg.PRBody, prBodyFormatReferenceLine) {
		t.Fatalf("PRBody = %q, want standardized PR format reference link", cfg.PRBody)
	}
	if !strings.Contains(cfg.PRBody, prBodySummaryHeading+"\n"+prBodyDefaultSummary) {
		t.Fatalf("PRBody = %q, want standardized agent summary block", cfg.PRBody)
	}
...
func TestApplyDefaultsAppendsMoltenBotHubFooterToCustomPRBody(t *testing.T) {
	...
	if !strings.Contains(cfg.PRBody, prBodySummaryHeading+"\nCustom PR body.") {
		t.Fatalf("PRBody = %q, want custom body normalized into agent summary block", cfg.PRBody)
	}
	if !strings.Contains(cfg.PRBody, "Original task prompt:\n```text\nfix tests\n```") {
		t.Fatalf("PRBody = %q, want original prompt block", cfg.PRBody)
	}
...
func TestApplyDefaultsKeepsSingleStandardizedPRFormatMarkers(t *testing.T) {
	t.Parallel()
	...
	if got := strings.Count(cfg.PRBody, prBodyFormatReferenceLine); got != 1 {
		t.Fatalf("PRBody contains %d format-reference markers, want 1: %q", got, cfg.PRBody)
	}
	if got := strings.Count(cfg.PRBody, prBodySummaryHeading); got != 1 {
		t.Fatalf("PRBody contains %d summary headings, want 1: %q", got, cfg.PRBody)
	}
}
