...
Reason: this app is fundamentally a local process orchestrator. It must run host tools (`git`, `gh`, `codex`/`claude`), manage local workspaces, and perform authenticated CLI flows. Those are native OS responsibilities and are already implemented in Go.
...
- `git`
- `gh` (GitHub CLI)
- selected agent CLI: `codex` or `claude`
